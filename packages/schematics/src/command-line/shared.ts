...
import {
  dependencies,
  Dependency
} from '@buildscale/schematics/src/command-line/affected-apps';
...
export function readDependencies(
  ...
  if (
    !fileExists(`${appRoot.path}/dist/buildscaledeps.json`) ||
    m > mtime(`${appRoot.path}/dist/buildscaledeps.json`)
  ) {
    const deps = dependencies(npmScope, projectNodes, f =>
      fs.readFileSync(`${appRoot.path}/${f}`, 'UTF-8')
    );
    fs.writeFileSync(
      `${appRoot.path}/dist/buildscaledeps.json`,
      JSON.stringify(deps, null, 2),
      'UTF-8'
    );
    return deps;
  } else {
    return readJsonFile(`${appRoot.path}/dist/buildscaledeps.json`);
  }
}
