...
describe('WorkspaceIntegrityChecks', () => {
  const packageJson = {
    dependencies: {
      '@buildscale/buildscale': '1.2.3'
    },
    devDependencies: {
      '@buildscale/schematics': '1.2.3'
    }
  };
  ...
  describe('package.json is consistent', () => {
    it('should not error when @buildscale/buildscale and @buildscale/schematics are in sync', () => {
      const c = new WorkspaceIntegrityChecks([], [], packageJson);
      expect(c.run().length).toEqual(0);
    });

    it('should error when @buildscale/buildscale and @buildscale/schematics are not in sync', () => {
      const c = new WorkspaceIntegrityChecks([], [], {
        dependencies: {
          '@buildscale/buildscale': '1.2.3'
        },
        devDependencies: {
          '@buildscale/schematics': '4.5.6'
        }
      });
      const errors = c.run();
      expect(errors.length).toEqual(1);
