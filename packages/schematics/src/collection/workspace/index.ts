...
function updatePackageJson() {
  return updateJsonInTree('package.json', packageJson => {
    ...
    if (!packageJson.dependencies['@buildscale/buildscale']) {
      packageJson.dependencies['@buildscale/buildscale'] = buildscaleVersion;
    }
    ...
    if (!packageJson.devDependencies['@buildscale/schematics']) {
      packageJson.devDependencies['@buildscale/schematics'] = schematicsVersion;
    }
...
function updateAngularCLIJson(options: Schema): Rule {
  return updateJsonInTree('.angular-cli.json', angularCliJson => {
    ...
    angularCliJson.defaults.schematics['collection'] = '@buildscale/schematics';
...
function updateKarmaConf() {
  return (host: Tree, context: SchematicContext) => {
    ...
    const change = new InsertChange(
      ...
      stripIndents`
        const { makeSureNoAppIsSelected } = require('@buildscale/schematics/src/utils/cli-config-utils');
  ...
  return (host: Tree) => {
    ...
    const updatedConf = protractorConf
      ...
      .replace(
        `exports.config = {`,
        `
const { getAppDirectoryUsingCliConfig } = require('@buildscale/schematics/src/utils/cli-config-utils');
