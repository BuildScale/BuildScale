...
describe('app', () => {
  ...
  describe('not nested', () => {
    it('should update angular-cli.json', () => {
      const tree = schematicRunner.runSchematic(
        'app',
        { name: 'myApp', npmScope: 'buildscale' },
        appTree
      );
  ...
  describe('nested', () => {
    it('should update angular-cli.json', () => {
      const tree = schematicRunner.runSchematic(
        'app',
        { name: 'myApp', npmScope: 'buildscale', directory: 'myDir' },
        appTree
      );
      const updatedAngularCLIJson = getAngularCliConfig(tree);
      expect(updatedAngularCLIJson.apps).toEqual([
  ...
  it('should import NgModule', () => {
    ...
    expect(
      getFileContent(tree, 'apps/my-dir/my-app/src/app/app.module.ts')
    ).toContain('BuildScaleModule.forRoot()');
  });
  ...
  describe('tags', () => {
    it('should split tags by a comma', () => {
      const tree = schematicRunner.runSchematic(
        'app',
        { name: 'myApp', npmScope: 'buildscale', tags: 'one,two' },
        appTree
      );
      const updatedAngularCLIJson = getAngularCliConfig(tree);
